"""
Segmentation Model for Fruit Ripeness Detection System
"""
import torch
...
class AttentionGate(nn.Module):
    ...
    def forward(self, g, x):
        ...
        # Ensure consistent interpolation mode
...
class StochasticFeaturePyramid(nn.Module):
    ...
    def forward(self, x):
        ...
        # First branch - direct path
        out1 = x
        
        # Second branch - convolutional path with dropout
        out2 = self.conv1(x)
        out2 = self.bn1(out2)
        out2 = F.relu(out2)
        ...
        if self.training:
            # More deterministic approach - use fixed blend instead of random choice
            # This provides more consistent gradients
            return F.relu(out1 + out2 * 0.8)
        else:
            # During inference, use the combined output
...
class DynamicRegularization(nn.Module):
    def __init__(self, in_channels, reduction=16, min_channels=8):
        ...
        # L1 regularization strength (learnable)
    ...
    def forward(self, x):
        ...
        x = x * (0.8 + 0.2 * spatial_att)  # Limit the attention's impact range
    
        # Clamp the L1 regularization strength to prevent extreme values
        if self.training:
...
class BoundaryRefinementModule(nn.Module):
    def __init__(self, in_channels):
        ...
        # Edge detection with separate convolutions
    ...
    def forward(self, x):
        ...
        # Residual connection
        return x + refined * 0.1  # Scale down the refinement to avoid overfitting

class UNetResNet50(nn.Module):
    def __init__(self, n_classes=1):
        super(UNetResNet50, self).__init__()
        
        # Load pretrained ResNet50 as encoder but freeze early layers
        self.resnet = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V2)
        
        # Freeze early layers to prevent overfitting
        for param in list(self.resnet.parameters())[:20]:  # Freeze first ~20 parameters
        ...
        # Stochastic Feature Pyramids (keeping these as required)
        ...
        # Dynamic regularization (keeping these as required)
    ...
    def forward(self, x):
        ...
        # Final processing with boundary refinement
